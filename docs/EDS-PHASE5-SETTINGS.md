# EDS Fase 5 — Configurações (Admin): Plano de Design e Implementação

Este documento especifica os componentes, comportamentos, tokens e estrutura de página para a Fase 5 da migração EDS (Configurações Admin). Todos os designs seguem os princípios do [SLDS 2](https://www.lightningdesignsystem.com/) adaptados para React, usando design tokens EDS (`--eds-g-*`).
...
A Fase 5 entrega as páginas de **Configurações Administrativas** para o shell EDS. Os admins da workspace podem:
...
type WorkspaceSettings = {
  ...
  displayName: string;
  logo: string | null;
  frontendPolicy: 'ALLOW_USER_CHOICE' | 'FORCE_TWENTY' | 'FORCE_EDS';
...
- **Política de frontend:** Select com `ALLOW_USER_CHOICE`, `FORCE_TWENTY`, `FORCE_EDS`
...
3. O componente usa apenas design tokens EDS
