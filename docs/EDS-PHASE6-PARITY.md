# EDS Fase 6 — Paridade Funcional: Plano de Design e Implementação

Este documento especifica os componentes, comportamentos, tokens e estrutura de página para a Fase 6 da migração EDS (Paridade Funcional). Todos os designs seguem os princípios do [SLDS 2](https://www.lightningdesignsystem.com/) adaptados para React, usando design tokens EDS (`--eds-g-*`).
...
A Fase 6 alcança **paridade funcional completa** entre o frontend EDS e o frontend Twenty padrão. Ao final desta fase:

- Todas as páginas existentes no Twenty padrão estão disponíveis no EDS
...
- O feature flag `IS_EDS_ENABLED` é habilitado por padrão para novas workspaces
...
### IS_EDS_ENABLED — Habilitado por Padrão

A partir da Fase 6, o feature flag `IS_EDS_ENABLED` é habilitado automaticamente para **novas workspaces** e **seeds de desenvolvimento**.

**Arquivos alterados:**

1. `packages/twenty-server/src/engine/workspace-manager/workspace-migration/constant/default-feature-flags.ts`
   - Adicionado `FeatureFlagKey.IS_EDS_ENABLED` ao array `DEFAULT_FEATURE_FLAGS`

2. `packages/twenty-server/src/engine/workspace-manager/dev-seeder/core/utils/seed-feature-flags.util.ts`
   - Adicionada entrada `IS_EDS_ENABLED: true` no seed de feature flags

> **Nota:** Para workspaces existentes, o flag deve ser habilitado manualmente via painel de administração ou diretamente no banco de dados (`UPDATE "featureFlag" SET value = true WHERE key = 'IS_EDS_ENABLED'`).
...
| Contraste mínimo 4.5:1 em todos os textos | Todas as páginas (tokens EDS garantem isso) |
...
- [x] `IS_EDS_ENABLED` habilitado por padrão para novas workspaces
- [x] Documentação desta fase criada em `docs/EDS-PHASE6-PARITY.md`
- [x] `docs/EDS-MIGRATION.md` atualizado para marcar Fase 6 como concluída
