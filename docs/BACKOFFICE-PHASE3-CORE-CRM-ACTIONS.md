# Erencio.com Backoffice Fase 3 — Ações Principais do CRM: Plano de Design e Implementação

Este documento especifica os componentes, comportamentos, tokens e estrutura de página para a Fase 3 da migração Erencio.com Backoffice (Ações Principais do CRM). Todos os designs seguem os princípios do [SLDS 2](https://www.lightningdesignsystem.com/) adaptados para React, usando design tokens Erencio.com Backoffice (`--backoffice-g-*`).
...
colorTokens.surfaceDragOver = 'var(--backoffice-g-color-brand-base-5, rgba(1,118,211,0.05))';

// Novas CSS custom properties no global.css
--backoffice-g-color-brand-base-5: rgba(1, 118, 211, 0.05);
```
...
3. Componentes usam apenas design tokens Erencio.com Backoffice — sem cores ou espaçamentos hardcoded
...
11. Este documento (BACKOFFICE-PHASE3-CORE-CRM-ACTIONS.md) atualizado com quaisquer desvios de implementação
12. BACKOFFICE-COMPONENTS.md atualizado com documentação dos novos componentes
